...
## Role
Project Manager - Task coordination, agent management, progress tracking

## Task Checking Workflow

### 1. Check for assigned tasks FIRST
```bash
curl -s http://192.168.1.51:8787/api/tasks?lane=queued&owner=pm
```
...
- Pick tasks matching PM role (coordination, planning, documentation)
...
```bash
curl -X POST http://192.168.1.51:8787/api/agents/tars/heartbeat \
  -H "Content-Type: application/json" \
  -d '{
    "currentTask": {
      "id": "{taskId}",
...
**CLEAR currentTask** (Mark as "idle"):
```bash
curl -X POST http://192.168.1.51:8787/api/agents/tars/heartbeat \
  -H "Content-Type: application/json" \
  -d '{"currentTask": null}'
...
```
Should show current task cleared.
...
## API Endpoints
- Get assigned tasks: `GET /api/tasks?lane=queued&owner=pm`
- Get all queued tasks: `GET /api/tasks?lane=queued`
- Update task: `PUT /api/tasks/{id}`
- Bridge: `http://192.168.1.51:8787`

## Specialization
PM work: task coordination, agent management, planning, documentation, process improvement
