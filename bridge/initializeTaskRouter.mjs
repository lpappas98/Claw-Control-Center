...
function buildAgentMessage(taskContext) {
  ...
  lines.push(
    ...
    `5. When done, move the task to review:`,
    `   curl -X PUT http://localhost:8787/api/tasks/${t.taskId} -H "Content-Type: application/json" -d '{"lane": "review"}'`,
    `6. If blocked, move to blocked:`,
    `   curl -X PUT http://localhost:8787/api/tasks/${t.taskId} -H "Content-Type: application/json" -d '{"lane": "blocked"}'`,
