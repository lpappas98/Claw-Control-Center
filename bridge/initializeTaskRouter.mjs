...
function buildAgentMessage(taskContext) {
  ...
  lines.push(
    ...
    ``,
    `### ⚠️ CRITICAL: NO TEST TASK CREATION`,
    ``,
    `**MANDATORY RULE:** DO NOT create test tasks in the main task system under ANY circumstances.`,
    ``,
    `- ❌ NEVER create tasks with titles like: "E2E Test", "Batch Task", "Card Test", "Overflow Test", etc.`,
    `- ❌ NEVER create tasks for testing, validation, or workflow verification`,
    `- ❌ NEVER create fake/dummy/placeholder tasks`,
    ``,
    `**Why:** Test tasks clutter the real board, waste agent bandwidth, and are unnecessary.`,
    `**Instead:** All testing happens in Playwright tests (tests/ directory) which create/cleanup their own data.`,
    ``,
