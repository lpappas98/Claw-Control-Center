...
function buildAgentMessage(taskContext) {
  ...
  lines.push(
    ...
    `   \`\`\`bash`,
    `   # Get commit hashes`,
    `   COMMITS=$(git log --format="%H" -n 3 --reverse | jq -R . | jq -s .)`,
    `   `,
    `   # Log work data (commits are required, test results optional)`,
    `   curl -X PUT http://localhost:8787/api/tasks/${t.taskId}/work \\`,
    `     -H "Content-Type: application/json" \\`,
    `     -d "{\"commits\": $COMMITS, \"agentId\": \"${t.owner || 'agent'}\"}"`,
    `   \`\`\``,
    ``,
    ...
    `   curl -X PUT http://localhost:8787/api/tasks/${t.taskId}/work \\`,
    `     -H "Content-Type: application/json" \\`,
    `     -d '{"commits": ["<hash>"], "testResults": {"passed": 5, "failed": 0, "coverage": "85%"}, "agentId": "${t.owner || 'agent'}"}'`,
    `   \`\`\``,
    ``,
